...
- **Cursor Cloud Agent enablement for this fork.** `.cursor/environment.json` bootstraps Cloud Agents with `npm ci` and no `start` script (no servers to keep up). `AGENTS.md` now states this is the marceldarvas fork: live CLIs are Codex / Antigravity / OpenCode / Cline, the upstream Cursor CLI provider stays removed unless a human asks to restore it, and Cloud Agents validate with offline `npm test` only (`test:live` stays operator-local because it needs a signed-in Cline CLI). `ARCHITECTURE.md` matches the registry (`--cli codex|antigravity|opencode|cline`).
