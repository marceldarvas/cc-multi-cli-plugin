...
- **cmd.exe argument quoting fails closed instead of emitting a re-parseable command.** `quoteWindowsShellArg` wrapped values in double quotes and escaped embedded quotes as `\\"` — but cmd.exe has no backslash escape, so `"a\\"b"` parsed as the quoted run `"a\\"` followed by a bare `b"`, letting a value escape its own quoting. A `CURSOR_AGENT_PATH` shaped like `C:/x" & calc & "agent.cmd` could therefore smuggle a second command through the Windows `.bat`/`.cmd` spawn path used by every `runCommand` caller. Embedded quotes, a matched `%VAR%` pair (cmd.exe expands those *inside* double quotes, silently rewriting the value), and control characters are now refused with a specific error rather than mangled. A **lone** `%` is still allowed: `%` is a legal Windows filename character (the reserved set is `< > : " / \ | ? *` plus control chars), and only a matched pair can expand — refusing every `%` would have broken paths like `C:/100% funded/agent.cmd` that worked before. The refusal surfaces through `runCommand` as `result.error` rather than as a throw, because `adapter.isAvailable()` and its callers must return a shape; a throw there would have converted a security fix into a crash. `resolveSpawnCommand` now takes an injectable `platform`, so the Windows branch is reachable from tests on a POSIX host — it was untestable before, which is how the quoting bug survived.
- **The three copies of the numeric-env parser are one.** `parseTimeoutSecs` in `cursor.mjs` and `cline.mjs` and `envWindowMs` in `lib/acp/client.mjs` were near-identical, and differed in a way that mattered: the timeouts accepted an explicit `0` as a real deadline while the ACP windows treated `0` as unset. Both semantics are preserved behind `envNumber(raw, fallback, { allowZero })` in `lib/env.mjs`, with the difference documented at each call site instead of left implicit.
...
- **Cursor headless turns are bounded by a watchdog.** `runHeadlessCursorTurn` spawned `agent -p` with no timeout of any kind, so a wedged agent hung the job forever — the Cursor adapter was the only one not honoring the watchdog contract the other adapters use. It now resolves `timeoutSec` (default 300 s, `CURSOR_TIMEOUT_SECS`, explicit `0` respected) plus 10 s slack, spawns detached so the process *group* can be killed, and on expiry runs `terminateProcessTree` and returns a `CursorTimeout` error. `--timeout-sec` now threads from `task.mjs` into the Cursor path. Regression test spawns a stub agent that sleeps and asserts the turn returns in ~1 s instead of hanging, and a second test asserts the child is actually dead rather than merely abandoned.

  The child is deliberately **not** spawned detached. Detaching would let the watchdog group-kill it, but `jobs.mjs` cancels by signalling the *worker's* process group, and a detached child is no longer in it — a cancelled Cursor job would keep running under `--force --trust` and keep writing files. Verified: a detached child lands in its own process group, so the worker-group kill cannot reach it. Staying in the worker's group keeps cancel correct and the watchdog takes `terminateProcessTree`'s bare-pid fallback. Consequence, documented rather than hidden: the watchdog kills the CLI itself, while descendants the CLI spawned are reaped by cancel. Closing that needs the child pid tracked in job state.
- **Cursor availability and auth probes no longer build a shell command string.** `getCursorAvailability` and `getCursorAuthStatus` ran `execSync("\"${cli}\" --version")`, and double quotes do not stop `$(…)`, so an operator-set `CURSOR_AGENT_PATH` was evaluated by the shell. Both go through `runCommand` now, which passes an argv array on POSIX and routes Windows `.bat`/`.cmd` through cmd.exe with quoted args — Cursor ships an `agent.cmd` wrapper, which a bare `execFile` cannot launch at all. `runCommand` gained a `timeout` passthrough so the probes keep their 8 s / 10 s bounds. Test asserts a path containing `$(touch …)` does not create the file.
- **A malformed `CURSOR_TIMEOUT_SECS` no longer becomes a 0 ms watchdog.** `Number(env || 300)` yields `NaN` for a typo'd value, and `setTimeout(fn, NaN)` coerces to a 1 ms timer (measured firing at 5 ms), so every Cursor turn would have been killed the moment it started. The value is validated with `Number.isFinite` before adoption; negative values fall back for the same reason. (`CURSOR_TIMEOUT_SECS=0` was never affected — env values are strings and `"0"` is truthy, so the `||` never substituted the default.)
- **`cline-diff` index assertions actually inspect the temp repo.** `indexHash` fed the relative path from `git rev-parse --git-path index` straight to `readFileSync`, which resolves against `process.cwd()` — so two tests hashed the *outer* repo's index against itself and asserted nothing, then crashed with `ENOTDIR` in a linked worktree (where `.git` is a file). The helper resolves against the repo under test, and now compares the staged tree and staged entries rather than the index file's bytes: a read-only `git diff` legitimately refreshes the index stat cache, so byte equality was never the right invariant. Suite verified green from both a normal clone and a linked worktree.
