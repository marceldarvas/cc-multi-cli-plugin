...
- **Cursor availability and auth probes no longer build a shell command string.** `getCursorAvailability` and `getCursorAuthStatus` ran `execSync("\"${cli}\" --version")`, and double quotes do not stop `$(…)`, so an operator-set `CURSOR_AGENT_PATH` was evaluated by the shell. Both use `execFileSync` with an argument array now. Test asserts a path containing `$(touch …)` does not create the file.
