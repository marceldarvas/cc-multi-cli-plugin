...
- **A malformed `CURSOR_TIMEOUT_SECS` no longer becomes a 0 ms watchdog.** `Number(env || 300)` yields `NaN` for a typo'd value, and `setTimeout(fn, NaN)` fires on the next tick, so every Cursor turn would have been killed the moment it started. The value is validated with `Number.isFinite` before adoption, and an explicit `0` is now honored rather than swallowed by the `||`.
